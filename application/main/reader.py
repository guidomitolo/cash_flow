...
def check_db(data_input, signal):
    ...
    if signal:
        ...
        all_db = None
        if query:
            all_db = [[line.bank, line.timestamp.date(), line.creditor, line.share, line.ars , line.usd, line.card_summary] for line in query]
...
def parse_data(account_num=None, table=None, close_date=None):
    ...
    if account_num:
        ...
        if close_date:
            last_date += [dt.strptime(close_date, '%Y-%m-%d') for i in range(len(table['Fecha']))]
        else:
            last_date += [None for i in range(len(table['Fecha']))]
        ...
        for i, j in zip(table['Importe_en_U$S'], table['Establecimiento']):
            if j != 'None':
                if i is not None and i != 'None':
                    ...
                    amount_usd.append(float(0))

    if balance:
        all_data = bancos, cuentas, date, description, flow, balance
        signal = 1
    else:
        all_data = bancos, date, creditor, share, amount_ars, amount_usd, last_date
        signal = 0
